...
- `npm test`: **212/212 tests passed across 21 Vitest files**.
...
- `npm run test:simulation-budget`: **1/1 passed**; the final isolated run completed in 552 ms and the test body took 122 ms, below the two-second gate.
...
| Presentation JavaScript | 35,510 bytes | 36,000 bytes |
| Lazy tactical runtime JavaScript | 894,157 bytes | 950,000 bytes |
| Model-0.4 runtime JavaScript | 90,171 bytes | 100,000 bytes |
| Original core JavaScript | 559,521 bytes | 575,000 bytes |
| Total JavaScript | 1,489,188 bytes | 1,550,000 bytes |
| Creature roster | 120,725 bytes | 125,000 bytes |
| Core CSS | 25,605 bytes | 26,000 bytes |
| Lazy reconstruction CSS | 2,998 bytes | 4,000 bytes |
| Total CSS | 28,603 bytes | 31,000 bytes |
| Original core deployable payload | 816,214 bytes | 835,000 bytes |
| Total deployable payload | 1,748,879 bytes | 1,850,000 bytes |
| Social image | 238,563 bytes | 300,000 bytes |

Vite output included the 120.72 kB creature JSON, 25.61 kB core CSS, 3.00 kB lazy reconstruction CSS, 35.51 kB presentation UI/storyboard JavaScript, 90.17 kB model-0.4 runtime, 450.72 kB entry JavaScript and an 894.16 kB raw / 238.35 kB gzip lazy tactical chunk. The reviewed ceilings, optimization and headroom are recorded in `MODEL_0.4_BUILD_BUDGET.md`.

## Browser, accessibility and deployment status

The exact production build completed the full desktop-Chromium suite with **36 passed, 1 expected project-scope skip and 0 failures**, then the complete reconstruction/accessibility suites in 360 px mobile Chromium, desktop Firefox and desktop WebKit with **45/45 passed**. The skipped desktop test is the mobile-only 360 px contract. The three-project matrix used one worker to avoid browser-resource contention.
...
- Likely-battle/tactical axe, captions, complete transcript, timeline keyboard controls, reduced-motion start state, no-WebGL fallback, conceptual fallback and lazy-scene loading: **passed in all four projects**.
...
- Visual evidence captured from the 0.5.0 production build at `output/playwright/likely-battle.png` and `output/playwright/tactical-reconstruction.png`; both result experiences were legible and the browser console reported zero errors.
...
- Primitive archetypes are tactical tokens; there are no bespoke glTF models, touch orbit controls, audio or video/still scene export.
