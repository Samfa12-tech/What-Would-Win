...
- Added rules-of-engagement inputs for mindset, win condition, prior knowledge, awareness, facing, arena boundary, water depth, group doctrine and casualty tolerance, with bounded deterministic effects and directional regression tests.
- Added disclosure-only specimen basis and sex declarations without introducing unsupported coefficient precision.
- Expanded the calibration suite from seven to twelve scenarios and documented the non-superhero community research that informed roster and methodology priorities.
- Advanced model/data versions to 0.2.0 and the compact share format to v3, retaining deployed v2, v1 and unversioned migration paths.
- Raised the raw production asset guardrail to 575 kB JavaScript and 700 kB total to accommodate the larger built-in roster; gzip output remains under the 150 kB JavaScript target.
...
- Retained explicit migration for deployed compact v2, v1-envelope and delivered unversioned links.
